...
const validateContact = (contact: string) => {
  ...
  if (c.length !== 11) return "Phone number must be exactly 11 digits.";
  if (!/^09\d{9}$/.test(c)) return "Phone number must start with '09'.";
  return "";
};
...
const validateNumber = (value: number, fieldName: string) =>
  (!value || value <= 0) ? `${fieldName} is required and must be greater than 0.` : "";
const validatePeriodOfResidency = (value: string) => {
  if (!value?.trim()) return "Period of residency is required.";
...
const ResidentRegistrationForm = ({ onBack }: ResidentRegistrationFormProps) => {
  ...
  // Street and zone stay editable; dob stays editable
  ...
  const validateCurrentStep = (): boolean => {
    ...
    switch (currentStep) {
      ...
      case 3:
        ...
        if (newErrors.resident_status || newErrors.period_of_residency || newErrors.voter_status || newErrors.emp_status || newErrors.pwd) isValid = false;
        break;
    }
    setErrors(newErrors);
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-5">
              <div>
                ...
                <FieldInput id="ext_name" placeholder="Jr., Sr." value={formData.ext_name} onChange={(e) => upd("ext_name", e.target.value)} disabled={isFieldDisabled("ext_name")} className={isFieldDisabled("ext_name") ? "cursor-not-allowed opacity-60" : ""} />
              </div>
              <div>
                <FieldLabel htmlFor="nick_name">Nickname</FieldLabel>
                <FieldInput id="nick_name" value={formData.nick_name} onChange={(e) => upd("nick_name", e.target.value)} disabled={isFieldDisabled("nick_name")} className={isFieldDisabled("nick_name") ? "cursor-not-allowed opacity-60" : ""} />
              </div>
              <div>
                <FieldLabel required>Sex</FieldLabel>
                <RadioGroup value={formData.sex} onValueChange={(v) => upd("sex", v)} className="flex gap-5 mt-2.5" disabled={isFieldDisabled("sex")}>
                  {["Male","Female"].map((s) => (
              ...
              <div>
                ...
                <Select value={formData.marital_status} onValueChange={(v) => upd("marital_status", v)} disabled={isFieldDisabled("marital_status")}>
                  <SelectTrigger {...ST} style={{ borderColor: errors.marital_status ? "#ef4444" : undefined }} className={`${ST.className} ${isFieldDisabled("marital_status") ? "cursor-not-allowed opacity-60" : ""}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
              <div>
                ...
                <FieldInput id="name_of_spouse" value={formData.name_of_spouse} onChange={(e) => upd("name_of_spouse", e.target.value)} disabled={isFieldDisabled("name_of_spouse")} className={isFieldDisabled("name_of_spouse") ? "cursor-not-allowed opacity-60" : ""} />
              </div>
              <div>
                <FieldLabel htmlFor="dob" required>Date of Birth</FieldLabel>
                <FieldInput id="dob" type="date" value={formData.dob} max={new Date().toISOString().split("T")[0]} onChange={(e) => upd("dob", e.target.value)} style={{ borderColor: errors.dob ? "#ef4444" : undefined }} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
              <div>
                ...
                <FieldInput id="pob" value={formData.pob} onChange={(e) => upd("pob", e.target.value)} style={{ borderColor: errors.pob ? "#ef4444" : undefined }} disabled={isFieldDisabled("pob")} className={isFieldDisabled("pob") ? "cursor-not-allowed opacity-60" : ""} />
                {errors.pob && <p className="mt-1 text-xs text-red-500">{errors.pob}</p>}
              </div>
              <div>
                <FieldLabel htmlFor="religion" required>Religion</FieldLabel>
                <FieldInput id="religion" value={formData.religion} onChange={(e) => upd("religion", e.target.value)} style={{ borderColor: errors.religion ? "#ef4444" : undefined }} disabled={isFieldDisabled("religion")} className={isFieldDisabled("religion") ? "cursor-not-allowed opacity-60" : ""} />
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-5">
              <div>
                ...
                <FieldInput id="height_cm" type="number" value={formData.height_cm || ""} onChange={(e) => upd("height_cm", e.target.value)} style={{ borderColor: errors.height_cm ? "#ef4444" : undefined }} disabled={isFieldDisabled("height_cm")} className={isFieldDisabled("height_cm") ? "cursor-not-allowed opacity-60" : ""} />
              ...
              <div>
                ...
                <FieldInput id="weight_kg" type="number" value={formData.weight_kg || ""} onChange={(e) => upd("weight_kg", e.target.value)} style={{ borderColor: errors.weight_kg ? "#ef4444" : undefined }} disabled={isFieldDisabled("weight_kg")} className={isFieldDisabled("weight_kg") ? "cursor-not-allowed opacity-60" : ""} />
              ...
              <div>
                ...
                <Select value={formData.blood_type} onValueChange={(v) => upd("blood_type", v)} disabled={isFieldDisabled("blood_type")}>
                  <SelectTrigger {...ST} style={{ borderColor: errors.blood_type ? "#ef4444" : undefined }} className={`${ST.className} ${isFieldDisabled("blood_type") ? "cursor-not-allowed opacity-60" : ""}`}>
                    <SelectValue placeholder="Select" />
                  </SelectTrigger>
                  <SelectContent className="z-50">
              ...
              <div>
                ...
                <Select value={formData.complexion} onValueChange={(v) => upd("complexion", v)} disabled={isFieldDisabled("complexion")}>
                  <SelectTrigger {...ST} style={{ borderColor: errors.complexion ? "#ef4444" : undefined }} className={`${ST.className} ${isFieldDisabled("complexion") ? "cursor-not-allowed opacity-60" : ""}`}>
                    <SelectValue placeholder="Select" />
                  </SelectTrigger>
                  <SelectContent className="z-50">
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="max-w-sm">
              ...
              <FieldInput id="phone_number" type="tel" placeholder="09XX XXX XXXX" value={formData.phone_number} onChange={(e) => upd("phone_number", e.target.value)} style={{ borderColor: errors.phone_number ? "#ef4444" : undefined }} disabled={isFieldDisabled("phone_number")} className={isFieldDisabled("phone_number") ? "cursor-not-allowed opacity-60" : ""} />
            ...
            <div className="max-w-sm">
              ...
              <FieldInput id="email_address" type="email" placeholder="your.email@example.com" value={formData.email_address} onChange={(e) => upd("email_address", e.target.value)} style={{ borderColor: errors.email_address ? "#ef4444" : undefined }} disabled={isFieldDisabled("email_address")} className={isFieldDisabled("email_address") ? "cursor-not-allowed opacity-60" : ""} />
        ...
        return (
          <div className="space-y-6">
            ...
            <div>
              ...
              <FieldInput id="house_block_lot_no" value={formData.house_block_lot_no} onChange={(e) => upd("house_block_lot_no", e.target.value)} style={{ borderColor: errors.house_block_lot_no ? "#ef4444" : undefined }} disabled={isFieldDisabled("house_block_lot_no")} className={isFieldDisabled("house_block_lot_no") ? "cursor-not-allowed opacity-60" : ""} />
                ...
                <div className="relative">
                  <Select value={formData.street} onValueChange={(v) => { setFormData((p) => ({ ...p, street: toUpperCase(v) })); setErrors((prev) => ({ ...prev, street: "" })); }}>
                    ...
                    <SelectContent className="max-h-60 z-50">
                      {streets.map((s) => (<SelectItem key={s.id} value={toUpperCase(s.name)}>{s.name}{s.formerly ? ` (formerly ${s.formerly})` : ""}</SelectItem>))}
                    </SelectContent>
                  </Select>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
              <div>
                ...
                <FieldInput id="house_owner" value={formData.house_owner} onChange={(e) => upd("house_owner", e.target.value)} disabled={isFieldDisabled("house_owner")} className={isFieldDisabled("house_owner") ? "cursor-not-allowed opacity-60" : ""} />
              </div>
              <div>
                ...
                <Select value={formData.relationship_to_owner} onValueChange={(v) => { setFormData((p) => ({ ...p, relationship_to_owner: v })); }} disabled={isFieldDisabled("relationship_to_owner")}>
                  <SelectTrigger {...ST} className={`${ST.className} ${isFieldDisabled("relationship_to_owner") ? "cursor-not-allowed opacity-60" : ""}`}><SelectValue placeholder="Select" /></SelectTrigger>
                  <SelectContent className="z-50">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
              <div>
                ...
                <Select value={formData.resident_status} onValueChange={(v) => upd("resident_status", v)} disabled={isFieldDisabled("resident_status")}>
                  <SelectTrigger {...ST} style={{ borderColor: errors.resident_status ? "#ef4444" : undefined }} className={`${ST.className} ${isFieldDisabled("resident_status") ? "cursor-not-allowed opacity-60" : ""}`}><SelectValue placeholder="Select" /></SelectTrigger>
                  <SelectContent className="z-50">
              ...
              <div>
                ...
                <FieldInput id="period_of_residency" placeholder="e.g., 5 years" value={formData.period_of_residency} onChange={(e) => upd("period_of_residency", e.target.value)} style={{ borderColor: errors.period_of_residency ? "#ef4444" : undefined }} disabled={isFieldDisabled("period_of_residency")} className={isFieldDisabled("period_of_residency") ? "cursor-not-allowed opacity-60" : ""} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
              <div>
                ...
                <RadioGroup value={formData.voter_status} onValueChange={(v) => upd("voter_status", v)} className="flex gap-5 mt-2.5" disabled={isFieldDisabled("voter_status")}>
              ...
              <div>
                ...
                <FieldInput id="precinct_no" value={formData.precinct_no} onChange={(e) => upd("precinct_no", e.target.value)} disabled={isFieldDisabled("precinct_no")} className={isFieldDisabled("precinct_no") ? "cursor-not-allowed opacity-60" : ""} />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-5">
              <div>
                ...
                <Select value={formData.emp_status} onValueChange={(v) => upd("emp_status", v)} disabled={isFieldDisabled("emp_status")}>
                  <SelectTrigger {...ST} style={{ borderColor: errors.emp_status ? "#ef4444" : undefined }} className={`${ST.className} ${isFieldDisabled("emp_status") ? "cursor-not-allowed opacity-60" : ""}`}><SelectValue placeholder="Select" /></SelectTrigger>
                  <SelectContent className="z-50">
              ...
              <div>
                ...
                <FieldInput id="occupation" value={formData.occupation} onChange={(e) => upd("occupation", e.target.value)} disabled={isFieldDisabled("occupation")} className={isFieldDisabled("occupation") ? "cursor-not-allowed opacity-60" : ""} />
              </div>
              <div>
                <FieldLabel htmlFor="position">Position</FieldLabel>
                <FieldInput id="position" value={formData.position} onChange={(e) => upd("position", e.target.value)} disabled={isFieldDisabled("position")} className={isFieldDisabled("position") ? "cursor-not-allowed opacity-60" : ""} />
              </div>
            </div>
            <div>
              <FieldLabel required>PWD Status</FieldLabel>
              <RadioGroup value={formData.pwd} onValueChange={(v) => upd("pwd", v)} className="flex gap-6 mt-2.5" disabled={isFieldDisabled("pwd")}>
                {["Yes","No"].map((opt) => (
            ...
            {imagePreview && (
              <div className="flex items-center gap-4 p-4" style={{ backgroundColor: "#f8faff", borderRadius: 2, border: "1px solid #dde3ed" }}>
                ...
                <div>
                  ...
                  <p className="text-sm text-foreground">{residentImage?.name ?? "Existing photo"}</p>
                </div>
              </div>
            )}
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: "rgba(0,0,0,0.55)" }}>
        <div className="w-full sm:max-w-lg md:max-w-xl overflow-hidden" style={{ borderRadius: "20px", backgroundColor: "white", boxShadow: "0 20px 25px -5px rgba(0,0,0,0.1)" }}>
          <div className="relative overflow-hidden px-6 pt-8 pb-6 text-center" style={{ backgroundColor: NAVY }}>
            ...
            <p className="text-sm relative z-10" style={{ color: "rgba(255,255,255,0.65)" }}>Resident Registration</p>
          </div>
          <div className="px-6 py-6">
            ...
            <p className="text-sm text-center mb-6 leading-relaxed" style={{ color: "#6b7280" }}>Your resident registration is being reviewed by the barangay office.</p>
            <button onClick={() => navigate(`/request/resident_registration/${successData.id}`)} className="w-full py-3 text-sm font-bold text-white rounded-lg mb-3" style={{ backgroundColor: NAVY }}>View My Request</button>
