      ...
      >
        ...
        >
          ...
          <p className="text-sm relative z-10" style={{ color: "rgba(255,255,255,0.65)" }}>
            Resident Registration & Appointment Scheduled
          </p>
        </div>
        ...
        <div className="px-6 py-6 space-y-4">
          ...
          <p className="text-sm text-center leading-relaxed" style={{ color: "#6b7280" }}>
            Your resident registration and appointment have been submitted. Please arrive 10 minutes early on your scheduled date.
          </p>
...
const ResidentRegistrationForm = ({ onBack }: ResidentRegistrationFormProps) => {
  ...
  const fetchAvailableSlots = async (date: string) => {
    ...
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_WEB_URL}/api/schedules/available-slots`,
        {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axios.post(`${import.meta.env.VITE_WEB_URL}/api/residents`, payload, { withCredentials: true });
