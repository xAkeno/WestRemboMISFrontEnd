...
const BarangayClearanceForm = ({ onBack }: BarangayClearanceFormProps) => {
  ...
  //    street-matching runs AFTER the streets list has loaded, identical
  //    to the working behavior in BarangayCertificateForm. ────────────────────
  useEffect(() => {
    ...
    loadUser();
  }, [streets]); // ← same as Certificate: re-runs when streets loads so matchedStreet resolves correctly
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-5">
              <div>
                ...
                <FieldInput
                  ...
                  className={isFieldDisabled("ext_name") ? "cursor-not-allowed opacity-60" : ""}
                />
              </div>
              <div>
        ...
        return (
          <div className="space-y-5">
            ...
            <ReviewCard title="Personal Information">
              <ReviewRow
                label="Full Name"
                value={`${formData.prefix} ${formData.first_name} ${formData.middle_name} ${formData.surname} ${formData.ext_name}`.trim()}
              />
